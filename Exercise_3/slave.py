...
def run_network(ntrain = 50000, nvalid = 10000, ntest = 10000, algorithm_type = 'SGD',
         batch_size_train = 500, batch_size_valid = 500, batch_size_test = 500,
         LR = 0.1, M = 0.9, nfilters = 32, time_limit = 5):
...
def main():
    ...
    while not end_flag:
        ...
        if success:
            num = line.split()
            # Train
            acc = run_network(nfilters=int(num[1]), batch_size_train=int(num[2]),M=float(num[3]), LR=float(num[4]), algorithm_type=num[5])
            s = '{}\t{}\n'.format(line, acc)
            # Save results
            print('Writing to results.txt %s' % s)
